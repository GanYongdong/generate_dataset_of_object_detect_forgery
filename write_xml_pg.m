function write_xml_pg(writefilename, size_, data, labelStrSet)
% 写xml文件，这个写的固定用于标准yolo标签xml文件，
% writefilename是保存的文件路径名称，
% size_是[width,height,depth]的向量，
% data是n行的数据，每行代表一个目标对象，第一列是类别标号（数字代表），17 18 19 20列分别是xmin ymin xmax ymax
% author: ganyongdong <1141951289@qq.com> 2021.3.8
...
for i = 1 : size(data, 1)
    % while
    % object
    xmin = round(data(i,17));
    ymin = round(data(i,18));
    xmax = round(data(i,19));
    ymax = round(data(i,20));
    pg_theta = data(i,21);
    ...
    nameNode_c.appendChild(nameNode_c_c);
    nameNode_c_c = docNode.createElement('pg_theta');
    nameNode_c_c.appendChild(docNode.createTextNode(sprintf('%d',pg_theta)));
    nameNode_c.appendChild(nameNode_c_c);
    nameNode.appendChild(nameNode_c);
    docRootNode.appendChild(nameNode);
end
