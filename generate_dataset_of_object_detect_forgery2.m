...
% 比如这个取证数据集，给定图片集，读取每一张图片，会随机取一块不规则区域，提取ROI进行对应操作，保存当前最大矩形框变量和操作后的图片，储存到本地文件
% 操作数目前是6，对应类别标签1-6，看懂m文件都是可以修改
% 输入输出目录和一些用户参数在用户区修改
...
imgDataOutPath = 'D:\MyStudyProj\generate_dataset_of_object_detect_forgery\datasetTmp'; %输出图像目录
...
count = 1;
% 预设label可选所属类别
labelStrSet = ["homoFilt"; "mediFilt"; "addNoise"; "histgrEq"; "gausFilt"; "prewShap"];
...
for picCount = 1:length(imgDataDir) % 遍历所有图片文件
    ...
    if factor_of_imgSrc_zoom ~= 1.0
        imgSrc = imresize(imgSrc, factor_of_imgSrc_zoom, 'bilinear');
    end
    ...
    % 生成5幅不同操作的图像，后边对应操作的就从这取图像
    img_gray_homoFilter = HomoFilter(imgGray, 2.2, 0.25, 2, homo_d0); %同态滤波
    img_gray_medfilt = medfilt2(imgGray, [kernelVal, kernelVal]);%median filtering
    imgTmp = im2double(imgGray); % 改为你要读入图片的路径;im2double作归一化处理
    imgTmp = imnoise(imgTmp, 'salt & pepper', saltAndPepper_density); %添加密度为5%的椒盐噪声
    img_gray_saltAndPepper = im2uint8(imgTmp);
    img_gray_histeq = histeq(imgGray);
    gausFilter = fspecial('gaussian',[kernelVal kernelVal],1); %高斯滤波
    img_gray_gausFilter = imfilter(imgGray, gausFilter, 'replicate'); %对任意类型数组或多维图像进行滤波
	img_gray_prewittSharp = imgGray + uint8(filter2(fspecial('prewitt'),imgGray)*sharp_factor); %prewitt锐化
